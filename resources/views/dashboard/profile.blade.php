...
@section('dashboard-content')
        ...
        <form method="POST" action="{{ route('profile.update') }}" class="space-y-4">
            ...
            <div>
                ...
                <input id="email" type="email" value="{{ $user->email }}" readonly
                    class="w-full px-4 py-3 rounded-btn border border-sage-dark/10 bg-cream/60 text-sage-dark/70 cursor-not-allowed" />
                <p class="mt-1 text-xs text-sage-dark/60">
                    El correo no se puede modificar. Si necesitas cambiarlo, contacta a soporte.
                </p>
            </div>

            <button type="submit"
                class="bg-sage text-white font-semibold px-6 py-3 rounded-btn hover:bg-sage-dark transition">
                Guardar cambios
            </button>
        </form>
    </div>
    ...
    <div class="bg-white rounded-card shadow-soft p-6">
        ...
        <form method="POST" action="{{ route('profile.password.update') }}" class="space-y-4">
            ...
            <div>
                <label for="password_confirmation" class="block text-sm font-semibold mb-2">Confirmar nueva
                    contraseña</label>
