    ...
    <div class="bg-white rounded-card shadow-soft p-4 sticky top-24">
        ...
        <nav class="space-y-1">
            <a href="{{ route('dashboard') }}"
               class="flex items-center gap-3 px-3 py-2 rounded-btn transition {{ $isActive('dashboard') }}">
                {{-- Icono: chart-bar --}}
            ...
            <a href="{{ route('dashboard.tickets') }}"
               class="flex items-center gap-3 px-3 py-2 rounded-btn transition {{ $isActive('dashboard.tickets') }}">
                {{-- Icono: ticket --}}
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 shrink-0">
            ...
            <a href="{{ route('dashboard.favorites') }}"
                class="flex items-center gap-3 px-3 py-2 rounded-btn transition {{ $isActive('dashboard.favorites') }}">
                    {{-- Icono corazón --}}
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 shrink-0">
            ...
            <a href="{{ route('dashboard.profile') }}"
               class="flex items-center gap-3 px-3 py-2 rounded-btn transition {{ $isActive('dashboard.profile') }}">
                {{-- Icono: user-circle --}}
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 shrink-0">
        ...
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit" class="w-full flex items-center gap-3 px-3 py-2 rounded-btn text-coral hover:bg-cream transition">
                {{-- Icono: arrow-right-on-rectangle --}}
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 shrink-0">
